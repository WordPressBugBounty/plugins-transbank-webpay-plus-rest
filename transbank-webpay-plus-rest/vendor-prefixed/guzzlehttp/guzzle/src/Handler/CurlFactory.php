...
namespace TransbankVendor\GuzzleHttp\Handler;

use TransbankVendor\GuzzleHttp\Exception\ConnectException;
use TransbankVendor\GuzzleHttp\Exception\RequestException;
use TransbankVendor\GuzzleHttp\Promise as P;
use TransbankVendor\GuzzleHttp\Promise\FulfilledPromise;
use TransbankVendor\GuzzleHttp\Promise\PromiseInterface;
use TransbankVendor\GuzzleHttp\Psr7\LazyOpenStream;
use TransbankVendor\GuzzleHttp\TransferStats;
use TransbankVendor\GuzzleHttp\Utils;
use TransbankVendor\Psr\Http\Message\RequestInterface;
use TransbankVendor\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \TransbankVendor\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \TransbankVendor\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \TransbankVendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \TransbankVendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
